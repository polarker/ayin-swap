...
import { TokenPairFactory, TokenPairTypes } from '../src/contracts/ts';
    ...
    async function test(tokenAId: string, tokenBId: string, tokens?: Token[]) {
      ...
      const testResult = await TokenPairFactory.tests.createPair({
        ...
        inputAssets: [
          {
            address: payer,
            asset: { alphAmount: oneAlph * 2n, tokens: inputAssetTokens },
          },
        ],
      });

      const [token0Id, token1Id] = sortTokens(tokenAId, tokenBId);
      const pairContractId = subContractId(
        fixture.contractId,
        token0Id + token1Id,
        0
      );
      const pairContractState = getContractState<TokenPairTypes.Fields>(
