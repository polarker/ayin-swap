...
function createFeeCollectorFactory(
  tokenPairFactoryId: string,
  contractId: string
) {
  const feeCollectorPerTokenPairTemplate =
    createFeeCollectorPerTokenPairTemplate();
  ...
  const contractState = FeeCollectorFactoryImpl.stateForTest(
    {
      feeCollectorPerTokenPairTemplateId:
        feeCollectorPerTokenPairTemplate.contractId,
  ...
  return new ContractFixture(
    contractState,
    feeCollectorPerTokenPairTemplate.states(),
    address
  );
}
...
describe('test fee collector', () => {
  ...
  beforeEach(async () => {
    ...
    tokenPairFactoryFixture = createTokenPairFactory(
      sender,
      feeCollectorFactoryId
    );
    feeCollectorFactoryFixture = createFeeCollectorFactory(
      tokenPairFactoryFixture.contractId,
      feeCollectorFactoryId
    );
    tokenPairFactoryFixture.dependencies.push(
      ...feeCollectorFactoryFixture.states()
    );
    tokenPairFixture = createTokenPair(
      token0Id,
      token1Id,
      undefined,
      tokenPairFactoryFixture
    );
  });

  async function mintAndSwap(
    ...
    const { contractState } = await mint(
      ...
      token0Amount,
      token1Amount,
      tokenPairState.fields,
      tokenPairState.asset
    );
    ...
    return getContractState<TokenPairTypes.Fields>(
  ...
  test('feeTo:off', async () => {
      ...
      inputAssets: [
        {
          ...
          asset: {
            alphAmount: ONE_ALPH,
            tokens: [
              { id: tokenPairFixture.contractId, amount: burnLiquidity },
            ],
          },
        },
      ],
    ...
    const contractState1 = getContractState<TokenPairTypes.Fields>(
  ...
  test('feeTo:on', async () => {
    const enableFeeCollectorResult =
      await TokenPairFactory.tests.enableFeeCollector({
        ...
        existingContracts: tokenPairFactoryFixture.dependencies.concat(
          tokenPairFixture.selfState
        ),
        testArgs: {
          tokenPair: tokenPairFixture.contractId,
          alphAmount: ONE_ALPH,
        },
        inputAssets: [
          {
    ...
    const contractState0 = getContractState<TokenPairTypes.Fields>(
      enableFeeCollectorResult.contracts,
      tokenPairFixture.contractId
    );
    const feeCollectorId = subContractId(
      feeCollectorFactoryFixture.contractId,
      tokenPairFixture.contractId,
      0
    );
    const feeCollectorState =
      getContractState<FeeCollectorPerTokenPairImplTypes.Fields>(
        enableFeeCollectorResult.contracts,
        feeCollectorId
      );
    ...
    const burnResult = await TokenPair.tests.burn({
      ...
      address: tokenPairFixture.address,
      existingContracts: tokenPairFixture.dependencies.concat([
        feeCollectorState,
      ]),
      testArgs: { sender: sender, liquidity: burnLiquidity },
      inputAssets: [
    ...
    const contractState2 = getContractState<TokenPairTypes.Fields>(
      burnResult.contracts,
      tokenPairFixture.contractId
    );
    expect(contractState2.fields.totalSupply).toEqual(
      minimumLiquidity + 249750499251388n
    );
    expect(contractBalanceOf(contractState2, token0Id)).toEqual(
      1000n + 249501683697445n
    );
    expect(contractBalanceOf(contractState2, token1Id)).toEqual(
      1000n + 250000187312969n
    );

    const newFeeCollectorState =
      getContractState<FeeCollectorPerTokenPairImplTypes.Fields>(
        burnResult.contracts,
        feeCollectorId
      );
    expect(
      contractBalanceOf(newFeeCollectorState, tokenPairFixture.contractId)
    ).toEqual(249750499251388n);
  });
});
