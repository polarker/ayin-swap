...
import { TokenPairFactory } from '../src/contracts/ts';
...
const deployFactory: DeployFunction<Settings> = async (
  ...
  if (groupOfAddress(network.settings.feeSetter) !== deployer.account.group) {
    throw new Error(
      `Invalid feeSetter, expected group ${deployer.account.group}`
    );
  }
  ...
  const result = await deployer.deployContract(TokenPairFactory, {
    initialFields: initialFields,
  });
  console.log(
    `TokenPairFactory contract address: ${result.contractInstance.address}, contract id: ${result.contractInstance.contractId}`
  );
};
