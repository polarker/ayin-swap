...
import * as foo from '@alephium/web3';
import { PrivateKeyWallet } from '@alephium/web3-wallet';
import { program } from 'commander';
import { randomInt } from 'crypto';
import { default as devnetDeployment } from '../src/contracts/.deployments.devnet.json';
import {
  AddLiquidity,
  CreatePair,
  GetToken,
...
//     for (
//       let token1Index = token0Index + 1;
//       token1Index < tokenIds.length;
//       token1Index++
//     ) {
...
//       console.log(
//         `Create pair succeed, token0Id: ${token0Id}, token1Id: ${token1Id}, tokenPairId: ${tokenPairId}`
//       );
...
//   for (
//     let token0Index = 1;
//     token0Index <= remainTokens.length - 1;
//     token0Index++
//   ) {
//     for (
//       let token1Index = token0Index + 1;
//       token1Index <= remainTokens.length;
//       token1Index++
//     ) {
...
//         env.network.privateKeys,
//         opts.keyIndex === undefined ? 0 : (opts.keyIndex as number)
//       );
//       const factoryId = devnetDeployment.contracts.TokenPairFactory.contractId;
//       console.log(
//         `ALPH token id: ${ALPH_TOKEN_ID}, address: ${addressFromContractId(
//           ALPH_TOKEN_ID
//         )}`
//       );
...
//   });
//
program.command('upgrade-staking').action(async () => {
  const env = await getEnv();
  const signer = getSigner(env.network.privateKeys, 0);
  const newBytecode = Staking.contract.bytecode;
  const staking = devnetDeployment.contracts.Staking.contractInstance;

  const imm = encodeU256(0n);
  const mut = encodeBool(false);

  await UpgradeStaking.execute(signer, {
    initialFields: {
      staking: staking.contractId,
      newBytecode,
      newEncodedImmFields: binToHex(imm),
      newEncodedMutFields: binToHex(mut),
    },
  });
});
